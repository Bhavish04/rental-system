...
import {
  BarChart2, Users, Shield, Trophy, FileText,
...
export default function AdminPage() {
  ...
  const { data: report, isLoading: repLoading } = useQuery(
    ...
    () => adminAPI.reports({ start, end, period: period <= 1 ? 'daily' : period <= 7 ? 'weekly' : 'monthly' })
                  .then(r => r.data),
    { staleTime: 60_000 }
  )
  ...
  const unsuspendMutation = useMutation(
    (id) => adminAPI.unsuspend(id),
    {
      onSuccess: () => { toast.success('User reactivated'); qc.invalidateQueries('admin-users') },
      onError: () => toast.error('Failed to unsuspend'),
    }
  )

  const runModeration = async () => {
    if (!moderateId.trim()) { toast.error('Enter a property ID'); return }
    setModLoading(true)
    setModResult(null)
    try {
      const { data } = await adminAPI.moderate(moderateId.trim())
      setModResult(data)
    } catch { toast.error('Moderation check failed') }
    finally { setModLoading(false) }
  }

  const approveMutation = useMutation(
    (id) => adminAPI.approve(id),
  ...
  const reportStats = report ? [
    { label: 'Total Bookings',  value: report.total_bookings,     icon: <BarChart2 size={16} />, color: 'text-accent' },
    { label: 'Confirmed',       value: report.confirmed_bookings, icon: <CheckCircle size={16} />, color: 'text-green' },
    { label: 'Revenue',         value: `₹${(report.total_revenue_inr || 0).toLocaleString('en-IN')}`, icon: <DollarSign size={16} />, color: 'text-teal' },
    { label: 'New Users',       value: report.new_users,          icon: <Users size={16} />, color: 'text-blue' },
  ] : []
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold font-display text-text1">Admin Portal</h1>
          </div>
          <p className="text-text3 text-sm">Reports · Leaderboard · Moderation · User Management</p>
        </div>
        ...
        <div className="space-y-6 animate-fade-in">
          {repLoading && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => <div key={i} className="card h-20 animate-pulse bg-card2" />)}
            </div>
          )}
          {report && (
            <>
              ...
              {/* Simple bar chart */}
              <div className="card">
                <h3 className="text-text1 font-semibold text-sm mb-4">Booking & Revenue Overview</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={[
                    ...
                    <XAxis dataKey="name" tick={{ fill: '#5c5c72', fontSize: 11 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#5c5c72', fontSize: 11 }} axisLine={false} tickLine={false} />
                    <Tooltip
                      contentStyle={{ background: '#1e1f28', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, fontSize: 12 }}
              ...
              <div className="flex gap-3">
                <p className="text-text3 text-xs self-center">
                  Period: last {period} days · {report.start_date?.slice(0,10)} → {report.end_date?.slice(0,10)}
                </p>
                <a
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {leaders.map((l, i) => (
                  <tr key={i} className="border-b border-border/50 hover:bg-bg3 transition-colors">
                    ...
                    <td className="p-4 text-text1 font-medium">
                      {l.full_name || l.title}
                      {l.city && <span className="text-text3 font-normal"> · {l.city}</span>}
                    </td>
                ...
                {!lbLoading && leaders.length === 0 && (
                  <tr><td colSpan={4} className="p-8 text-center text-text3 text-xs">No data yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 animate-fade-in">
          <div className="card space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-semibold text-text1">AI Content Moderation</h3>
            </div>
            <p className="text-text3 text-xs">
              Powered by Gemini — checks for discriminatory content, spam, and policy violations
            </p>
            <div>
            ...
            >
              {modLoading ? (
                <><span className="w-4 h-4 border-2 border-bg border-t-transparent rounded-full animate-spin" /> Checking…</>
              ) : (
          ...
          <div>
            {modResult ? (
              <div className={`card space-y-4 animate-slide-up border ${
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-text3 text-xs">{modResult.explanation}</p>
                  </div>
                </div>
                  ...
                  <div className="bg-red/5 border border-red/20 rounded-xl p-3">
                    ...
                    <ul className="space-y-1">
                      {modResult.flags.map((f, i) => (
                        <li key={i} className="text-red/80 text-xs flex gap-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`badge-status border text-xs px-2.5 py-0.5 ${
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => approveMutation.mutate(modResult.property_id)}
              ...
              <div className="card flex flex-col items-center justify-center text-center py-16 h-full">
                ...
                <p className="text-text2 text-sm">Enter a property ID to run AI moderation check</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="card p-0 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {users.map(u => (
                  <tr key={u.id} className="border-b border-border/50 hover:bg-bg3 transition-colors">
                    <td className="p-4">
                      <p className="text-text1 font-medium">{u.full_name}</p>
                      <p className="text-text3 text-xs">{u.email}</p>
                    </td>
                    <td className="p-4">
                      <span className={`badge-status border text-xs px-2 py-0.5 ${
                        u.role === 'admin'  ? 'bg-red/10 text-red border-red/30' :
                        u.role === 'owner'  ? 'bg-accent/10 text-accent border-accent/30' :
